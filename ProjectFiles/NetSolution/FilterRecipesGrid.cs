...
public class FilterRecipesGrid : BaseNetLogic
{
    ...
    public void Filter(string stationName, string variantName)
    {
        ...
        if (!string.IsNullOrWhiteSpace(variantName))
            conditions.Add($"variant_name = '{variantName.Replace("'", "''")}'");

        if (!string.IsNullOrWhiteSpace(stationName))
            conditions.Add($"station_name = '{stationName.Replace("'", "''")}'");

        string whereClause = conditions.Count > 0 ? "WHERE " + string.Join(" AND ", conditions) : "";
        string queryExecuted = $"SELECT * FROM view_recipes_summary {whereClause}";
